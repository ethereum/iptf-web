...
Blob retention is the operational caveat. EIP-4844 retains blob payloads for 4096 epochs (around 18 days). Each batch's 14-day dispute window fits inside that retention period when the batch lands close to the petition's close-at-block, so on-chain disputes against recent batches can run from consensus-retained blobs. Resolution can take longer: from any batch published before close, the cooldown plus the 14-day dispute window plus the 14-day resolver grace can already exceed 18 days, so timely resolution depends on a [voluntary archive](https://blobscan.com/) or long-term blob storage such as [EthStorage](https://ethstorage.io/). The on-chain batch SNARK commitments bind blob payloads to versioned hashes, so any archive copy is verifiable against the registry record; the trust assumption shifts from "the consensus layer retains blobs" to "at least one honest archive serves the bytes."
